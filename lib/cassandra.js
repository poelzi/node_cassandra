          ...
          while (i--) {
            ...
            if (col) {
              ...
              while (j--) {
                ...
                obj[key][self.parseColumn(col.name)][sub_col.name] = sub_col.value;
              }
            } else {
              // standard
              col = res[key][i].column;
              obj[key][self.parseColumn(col.name)] = col.value;
            }
          }
        }
      }
...
/**
 * Format date for column name before being sent to database
 * Can be overwritten to support more input/custom formats
 *
 * @param value data to be serialized
 * @API: public
 */
ColumnFamily.prototype.formatColumn = function(value) {
    if(!this.ready)
        throw new Error("not ready");
    if((this.comparator_type === 'org.apache.cassandra.db.marshal.LexicalUUIDType' ||
        this.comparator_type === 'org.apache.cassandra.db.marshal.TimeUUIDType') &&
        Guid.isGuid(value, true)) {
        if(value instanceof Guid)
            return value.bytes;
        return new Guid.create(value).bytes;
    }
    return value;
};

/**
 * Format date for column name in database connections
 * Parse Database data and return designated js object
 *
 * @param value data the database returned
 * @API public
 */
ColumnFamily.prototype.parseColumn = function(value) {
    if(!this.ready)
        throw new Error("not ready");
    if(this.comparator_type === 'org.apache.cassandra.db.marshal.LexicalUUIDType' ||
        this.comparator_type === 'org.apache.cassandra.db.marshal.TimeUUIDType') {
        rv = new Guid.create(value);
        return rv
    }
  ...
  if (columns_or_options) {
    ...
    if (typeof columns_or_options.slice === 'function') {
      ...
      if (!(columns instanceof Array)) {
        ...
        var self = this;
        columns = columns.filter(function(x) { return self.formatColumn(x); });
      }
      predicate.column_names = columns;
      options = args.shift() || default_options;
    } else {
